...
if ($err) {
	...
	foreach($arrayEstados['divisions'] as $estado){
		...
		if($num_rows != 0){
			...
			$result3 = $conexao->sql("UPDATE sys_estado_bkp SET tl_estado = '$tl_estado',
																sg_estado = '$sigla',
																ds_iso = '$iso',
																fl_atualizado = '1'
			...
			$result3 = $conexao->sql("INSERT INTO sys_estado_bkp (cd_pais, tl_estado, sg_estado, ds_iso, fl_atualizado) VALUES ('$codigo_pais','$tl_estado','$sigla','$iso','1')");
...
$xml_output .= "</atualiza_paises>\n";
echo $xml_output;
//SELECT e.*, p.sg_pais FROM sys_estado_bkp e, sys_paises p WHERE e.cd_pais = p.cd_pais GROUP BY cd_pais ORDER BY `cd_pais`
?>